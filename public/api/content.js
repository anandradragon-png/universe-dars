...
async function handleOracle(req, res) {
  ...
  // ===========================================================
  // НОВАЯ МЕХАНИКА ОРАКУЛА (29.05.2026)
  // mode === 'card' = тарологический расклад: юзер задаёт вопрос,
  // система случайно вытягивает один из 81 Дара (64 базовых + 17
...
- Никакой латиницы и иностранных слов.
- Не используй штампы: «Я вижу», «знаки небес», «звёзды говорят», «карта предвещает».

МЯГКАЯ ПОДАЧА (важно):
- НИКОГДА не пиши «нужно», «надо», «должен/должна», «обязан», «требуется».
- Пиши «важно», «полезно», «попробуй», «обрати внимание», «открывается возможность».

ПРОТИВ ПОВТОРОВ:
- Не используй одну конструкцию 2-3 раза подряд. Варьируй: «Сейчас важно…», «Попробуй…», «Полезно…», «Открывается…», «Ключ — в…».

ФОРМАТ: только валидный JSON без markdown.`;

    userPrompt = `Вопрос юзера: "${user_query}"

Выпавшая Карта (характеристики, используй как образ для ответа, НЕ называя её по имени):
...
- Используй имя ${relative_name} в точности как дано. НЕ выдумывай и НЕ меняй фамилию, НЕ заменяй её другой. Можно естественно склонять имя по падежам, но сама фамилия остаётся той же.
...
ТВОЯ ЗАДАЧА:
По качествам дня близкого человека пользователя составь послание на сегодня - о ${relationLabel === 'близкий человек' ? 'близком' : relationLabel} по имени ${relative_name}:
1. ПРЕДСКАЗАНИЕ (prophecy) в ТРЕТЬЕМ ЛИЦЕ о ${relative_name}: "Сегодня ${relative_name}...", "У ${relative_name} открывается...". 3-5 предложений, поэтично и тепло.
2. ПРАКТИКА (practice) - как пользователь МОЖЕТ ПОДДЕРЖАТЬ ${relative_name} сегодня (мягкое слово, забота, внимание, конкретное действие). 2-3 предложения.
3. ЭНЕРГИИ (energies) - 4-5 коротких маркеров (по 2-3 слова) о ресурсах, доступных сегодня ${relative_name}.

${genderBlock}

СТРОГИЕ ПРАВИЛА:
- Говори про ${relative_name} ТОЛЬКО в третьем лице, обращаясь к читателю на "ты": "Твоя ${relationLabel} сегодня...", "У ${relative_name} открывается...". НИКОГДА не пиши о близком от первого или второго лица ("ты стоишь", "тебе дано") - это про самого читателя, а не про ${relative_name}.
- Используй имя ${relative_name} в точности как дано. НЕ выдумывай и НЕ меняй фамилию, НЕ заменяй её другой. Можно естественно склонять имя по падежам, но сама фамилия остаётся той же.
- Пиши только на грамматически безупречном литературном русском.
- Не используй длинное тире, только дефис или запятую.
- Не пиши иностранных слов, ни одной латинской буквы.
- НЕ называй дар, коды, поля, формулы, МА/ЖИ/КУН.
- Не разбивай день на утро/день/вечер.
- Не используй театральщину типа "Я вижу", "знаки небес".
- В энергиях дня - только светлые качества, не тени.
...
ФОРМАТ: только валидный JSON без markdown.`;
    ...
    if (isRelative) {
      userPrompt = `${contextBlock}\n\nСоставь послание на сегодня о ${relationLabel === 'близкий человек' ? 'близком' : relationLabel} пользователя по имени ${relative_name}. Веди речь о ${relative_name} в ТРЕТЬЕМ лице (не о читателе). Верни ТОЛЬКО JSON:\n{"prophecy":"предсказание в третьем лице о ${relative_name}","practice":"как пользователь может поддержать ${relative_name} сегодня","energies":["...","...","...","..."]}`;
...
// Баннер технических работ. Управляется env-переменной MAINTENANCE_MESSAGE на сервере.
