...
const DIARY_I18N = {
  ru: {
    ...
    // high_action: 3 варианта, ротируются по дню недели через getTipVariant()
    'tip.high_action':   'Сегодня ты в потоке. Выбери самое важное и сделай именно это — силы есть.',
    'tip.high_action_1': 'Окно силы открыто. Возьми одно главное дело и направь сюда всю энергию.',
    'tip.high_action_2': 'Энергия на максимуме. Один шаг к главной цели — и сегодняшний день будет твоим.',
    // high_rest: 2 варианта
    'tip.high_rest':    'Энергия есть, но тянет в покой? Послушай — иногда сила копится для завтра.',
    'tip.high_rest_1':  'Высокий заряд и желание остановиться — это не противоречие. Тело знает, когда копить.',
    // high_people: 2 варианта
    'tip.high_people':  'Ты как магнит сейчас. Иди к людям — встреча сегодня будет важной.',
    'tip.high_people_1':'В такой день даже случайный разговор может оказаться нужным. Открывайся.',
    // high_alone: 2 варианта
    'tip.high_alone':   'Энергии много, но хочется в одиночество — это творческое состояние. Включи проект, который только твой.',
    'tip.high_alone_1': 'Много сил и желание побыть с собой — редкое сочетание. Используй его для чего-то своего.',
    // mid_action: 2 варианта
    'tip.mid_action':   'Ровный фон. Самое время для рутины — то, что давно откладываешь, сегодня пойдёт.',
    'tip.mid_action_1': 'Устойчивый день. Делай привычные вещи чуть лучше обычного — этого достаточно.',
...
function showMirror(entry, dar) {
  ...
  // Ротация по дню недели: сначала пробуем вариант _1, _2 — чтобы подсказка
  // не повторялась одинаковой при одинаковом выборе энергии/направления.
  const _dayVariant = new Date().getDay(); // 0=вс..6=сб
  const _tip1 = t(tipKey + '_1') || '';
  const _tip2 = t(tipKey + '_2') || '';
  const _tipVariants = [t(tipKey) || '', _tip1, _tip2].filter(Boolean);
  const tip = _tipVariants[_dayVariant % _tipVariants.length] || '';
