...
2. НЕ обвиняй родителей ("вы делаете не так" — ЗАПРЕЩЕНО).
3. Используй имя ${child.name} естественно (не в каждом предложении, 3-5 раз за текст).
4. Привязывай ВСЁ к возрасту ${agePeriod.period} — что именно происходит СЕЙЧАС.
5. Давай КОНКРЕТНЫЕ бытовые примеры (не абстрактные советы).
6. Гендерные формы по полу: ${genderWord}, ${genderPron}.
7. Пиши на БЕЗУПРЕЧНОМ русском языке. Не используй длинное тире.
8. НЕ упоминай код дара, поля, формулы, МА/ЖИ/КУН.
9. НЕ используй слова "дар" в духовном смысле. Говори "его природа", "его сила", "то что заложено", "его суперспособность".
10. Каждый абзац должен нести ЦЕННОСТЬ — никакой воды.`,
